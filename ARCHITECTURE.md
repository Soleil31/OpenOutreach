...
- **Campaign** (`linkedin/models.py`) — `name` (unique), `users` (M2M to User), `product_docs`, `campaign_objective`, `booking_link`, `is_freemium`, `outreach_enabled` (off = base mode: still qualifies, i.e. still opens profiles), `active` (off = invisible to the daemon: no tasks, no seeding), `action_fraction`, `seed_public_ids` (JSONField).
- **LinkedInProfile** (`linkedin/models.py`) — 1:1 with User. `self_lead` FK to Lead (nullable, set on first self-profile discovery). Credentials, rate limits (`connect_daily_limit`, `connect_weekly_limit`, `follow_up_daily_limit`, and the profile-view budget `profile_view_daily_limit` / `profile_view_hourly_limit`, enforced by `profile_budget.py` rather than `can_execute`). Methods: `can_execute`/`record_action`/`mark_exhausted`. In-memory `_exhausted` dict for daily rate limit caching.
...
- **ProfileView** (`linkedin/models.py`) — one row per Voyager profile request, FK to LinkedInProfile, index on `(linkedin_profile, created_at)`. Separate from ActionLog because the deadman check and the client report treat ActionLog as outreach.
...
- **`profile_budget.py`** — `check()` before every Voyager profile request (raises `ProfileViewLimitReached` with `retry_after`), `record()` after it. Limits are re-read from the DB on every check, so an admin edit applies to the next profile.
