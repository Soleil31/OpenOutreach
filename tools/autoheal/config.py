...
# Классы, по которым модуль обязан положить в инцидент черновой разбор: гипотезу,
# ссылки на улики и предлагаемую правку. Правку он НЕ применяет — доказать её
# нечем, оракул есть только у селекторов. Смысл в том, чтобы к моменту, когда
# человек откроет инцидент, за него уже были прочитаны стеки, счётчики и логи.
DIAGNOSABLE_REASONS = ("wedge", "unknown")
