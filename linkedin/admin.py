...
class CampaignAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Posting — Prompt templates (advanced)", {
            "description": (
                "Здесь живут шаблоны промптов которые AI получает на вход "
                "когда пишет посты и обложки. Эти поля заполнены готовыми "
                "рабочими дефолтами — можно ничего не трогать. Если "
                "хочешь поменять структуру промпта (например, попросить "
                "AI писать жёстче, или добавить новые правила), правь "
                "прямо здесь. Описание подстановок — под каждым полем. "
                "Чтобы вернуть исходный дефолт — очисти поле и сохрани."
            ),
            "fields": (
