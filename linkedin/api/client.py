...
class PlaywrightLinkedinAPI:
    ...
    #
    # Voyager endpoints, base: https://www.linkedin.com/voyager/api
    #   POST /voyagerMediaUploadMetadata?action=upload   → register image
    #   PUT  <returned uploadUrl>                        → push bytes
    #   POST /contentCreation/normShares                 → create the post
    #
    # Endpoint shapes verified against:
    # https://github.com/RyanLisse/LinkedFish (POST_CREATION_SUMMARY.md +
    # Sources/LinkLion/LinkedInClient.swift). The Swift client and ours
    # share the same Voyager contract; only the transport differs.

    _BASE = "https://www.linkedin.com/voyager/api"

    def register_image_upload(
        self, file_size: int, filename: str,
    ) -> tuple[str, str]:
        """Register a feed-share image upload with LinkedIn.

        Returns ``(upload_url, media_urn)``. The upload URL is a one-shot
        signed endpoint (often on a CDN host); push the bytes there with
        ``put_image_bytes`` and then reference ``media_urn`` from the post
        body.
        """
        body = {
        ...
        res = self.post(
            f"{self._BASE}/voyagerMediaUploadMetadata?action=upload",
            headers={"content-type": "application/json"},
            data=json.dumps(body),
        )
        if not res.ok:
            raise IOError(
                f"register_image_upload failed: HTTP {res.status} — {res.text()[:400]}"
            )
        data = res.json()
        # Response shape (per LinkedFish + observed traffic):
        #   { "data": { "value": { "singleUploadUrl": "...", "urn": "urn:li:digitalmediaAsset:..." } } }
        # Some variants return the value bag at the top level — try both.
        value = (
        ...
        media_urn = (
            ...
            or value.get("mediaUrn")
        )
        if not upload_url or not media_urn:
            raise IOError(
                f"register_image_upload: unexpected response shape: {json.dumps(data)[:500]}"
            )
        return upload_url, media_urn

    _PUT_JS = """([url, b64, ctype, timeoutMs]) => {
    ...
    def put_image_bytes(
        self, upload_url: str, image_bytes: bytes,
        content_type: str = "image/png",
    ) -> None:
        """PUT raw image bytes to the LinkedIn-provided ``upload_url``.

        Runs inside the page (same browser fingerprint as our other Voyager
        calls). Bytes are shipped across the page.evaluate boundary as
        base64 because ``page.evaluate`` cannot pass binary directly.
        """
        b64 = base64.b64encode(image_bytes).decode("ascii")
        raw = self._run_with_watchdog(
            f"PUT image ({len(image_bytes)} bytes)",
            lambda: self.page.evaluate(
                self._PUT_JS,
                [upload_url, b64, content_type, self.timeout_ms],
            ),
        )
        if not raw.get("ok"):
            raise IOError(
                f"put_image_bytes failed: HTTP {raw.get('status')} — "
                f"{(raw.get('body') or '')[:400]}"
            )

    def create_post(self, text: str, media_urn: str | None = None) -> str | None:
        """Create a feed post via Voyager. Returns the new post URN if available.

        Pass ``media_urn`` (from ``register_image_upload``) to attach an
        image; omit it for a text-only post.
        """
        body: dict = {
        ...
        if media_urn:
            body["media"] = [{"category": "IMAGE", "mediaUrn": media_urn}]
        res = self.post(
            f"{self._BASE}/contentCreation/normShares",
            headers={"content-type": "application/json"},
            data=json.dumps(body),
        )
        if res.status == 401:
            raise AuthenticationError("Voyager create_post returned 401")
        if not res.ok:
            raise IOError(
                f"create_post failed: HTTP {res.status} — {res.text()[:500]}"
            )
        try:
            data = res.json()
        except Exception:
            return None
        # Response shape varies; typical: {"data": {"urn": "urn:li:share:..."}}
        return (
            data.get("data", {}).get("urn")
            or data.get("urn")
            or data.get("value", {}).get("urn")
        )
