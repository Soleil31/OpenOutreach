...
- **Follow-up dialogue**: `agents/follow_up.py` decides the conversation stage in Python — `_conversation_stage()` returns `opening` / `answer_first` / `qualify` / `advance` from the full `ChatMessage` history (scoped to `deal.creation_date`), and `follow_up_agent.j2` renders exactly one mode. The agent cannot judge this itself: it sees only `RECENT_MESSAGES_WINDOW` (6) messages and `chat_summary` stores no facts about our own turns. `answer_first` is checked first, so a lead asking "why are you asking me?" always gets an honest answer instead of a pitch. The qualifying question comes from `Campaign.qualifying_question` when an operator filled it, else `agents/follow_up_defaults.py:DEFAULT_QUALIFYING_QUESTION` — blank means "use the repo default", so the copy is not duplicated into every account's SQLite file. Repeated opening formulas are cut two ways: the prompt lists `previous_openers` and bans acknowledgment openers in `## Voice`, and `_strip_ack_opener()` removes them structurally after the decision (no LLM call, returns the original when only a fragment would remain).
